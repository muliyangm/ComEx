...
Normally, it requires the Internet accessibility to proceed; otherwise, you can pass `--offline` for logging locally, which can also be manually uploaded to [Wandb](https://wandb.ai/site) for further analysis.

## Commands
Recall that there is a supervised pretraining stage before the discovery phase. If you would like to train your model from scratch, please run the following lines to get a pretrained checkpoint on base classes. Note that your checkpoint will be located in the `checkpoints` folder by default. Please modify the arguments of `--dataset`, `--data_dir`, `--num_labeled_classes` and `--num_unlabeled_classes` for training on a different dataset; `--comment` is used to specify the [Wandb](https://wandb.ai/site) logging name.
```
...
After that, you can start novel class discovery using the pretrained checkpoint; otherwise, you can directly start with the above provided checkpoints:
```
